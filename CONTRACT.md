# benchkit contract v0.4 — what a spec repo and benchkit agree on
...
Contract version: `0.4` (manifest `benchkit: ">=0.4,<0.5"`).
...
Additive changes bump the minor (0.4); breaking changes bump the major. Spec repos declare what they were written against; benchkit refuses (exit 1) a manifest whose specifier excludes its own version.

**0.4** removed the `benchkit.align` helpers from the public surface. They were oracle-shape helpers for one benchmark family, had no importers, and contradicted this document's opening claim that benchkit knows nothing about any particular benchmark. Alignment logic of that kind belongs in the spec repo alongside the evaluators that use it.
